  ...
  if (message.content.charAt(0) === "!") {
    ...
    if (eggs) {
      ...
      if (message.content.toLowerCase().match(EasterEggCustomID.Norm) && !busy) {
        ...
        const imageUrl = message.attachments?.first()?.contentType?.startsWith("image/")
        ...
        if (conversationID) {
          try {
            convo = await openai.conversations.retrieve(conversationID);
          } catch {
        ...
        if (imageUrl) {
          completion = await openai.responses.create({
            ...
            input: [
              {
                content: [
                  { text: `${message.author.id} ${message.author.username}: ${message.content}`, type: "input_text" },
        ...
        console.info("Tokens: " + completion.usage?.total_tokens);
        const imageOutputs = completion.output.filter((o) => o.type == "image_generation_call");
        ...
        if (imageOutputs && imageOutputs.length > 0) {
          for (const image of imageOutputs) {
            ...
            if (image_base64 && image_base64.length > 0) {
              ...
              const reply = completion.output_text.slice(0, 1999);
        ...
        if (reply) {
          const newReply = reply.slice(0, 1999);
          await chatChannel.send(newReply);
        }

        const toolsUsed =
          completion.output
            .filter((o) => o.type.endsWith("_call"))
